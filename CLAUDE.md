...
- **Static themes** (Amber, Green, etc.): add/update an entry in `package.json#config.themes`, add a corresponding entry in `contributes.themes`, then run `npm run build`. Forgetting either half fails the build, so the two lists cannot drift. The order of `contributes.themes` is deliberately not checked, because it is load-bearing in exactly one place. It has no effect on *Preferences: Color Theme*, which groups by light/dark/high-contrast and sorts alphabetically by label within each group. What it does drive is the per-extension picker — `SetColorThemeAction`, reached from the Extensions view's gear menu → *Set Color Theme*, and the same list offered after installing — which lists only this extension's themes in unsorted registry order, i.e. manifest order. That picker live-previews whichever entry is focused (`onDidFocus` → `setColorTheme`, 100 ms debounce) and sets no `activeItems`, so focus lands on the first entry: it is applied on sight and accepted by Enter, with Escape reverting to the previous theme. That is why CRT Green is listed first.
